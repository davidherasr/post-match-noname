...
def build_report_payload(session: Session, report_id: int) -> dict:
    ...
    evaluated.sort(key=lambda row: row["evaluation"].general_rating or 0, reverse=True)
    noteworthy = [
        row for row in evaluated
        if row["evaluation"].standout
        or (row["evaluation"].recommendation or "") in {
            "Seguimiento recomendado", "Jugador interesante", "Prioridad de seguimiento"
        }
    ]
    standout_row = next(
        (row for row in rival if row["participation"].player_id == report.standout_player_id),
        None,
    )
    return {
        "report": report,
        ...
        "noteworthy_players": noteworthy,
        "standout_row": standout_row,
        "settings": settings,
    }


def report_filename(session: Session, report_id: int) -> str:
    payload = build_report_payload(session, report_id)
    match = payload["match"]
    report = payload["report"]
    date_str = match.match_date.strftime("%Y-%m-%d")
    return (
        f"PostMatch_{date_str}_{_slug(match.home_team.name)}_vs_"
        f"{_slug(match.away_team.name)}_V{report.version}.pdf"
    )
...
def generate_report_pdf(session: Session, report_id: int) -> bytes:
    payload = build_report_payload(session, report_id)
    report = payload["report"]
    match = payload["match"]
    ...
    def header_footer(canvas, current_doc):
        ...
        if page > 1:
            ...
            canvas.drawString(18 * mm, PAGE_H - 11.5 * mm, club_name.upper())
            canvas.setFillColor(HexColor("#64748B"))
            canvas.setFont("Helvetica", 7.2)
            canvas.drawRightString(
                PAGE_W - 18 * mm,
                PAGE_H - 11.5 * mm,
                f"{match.home_team.name} - {match.away_team.name} · {match.match_date.strftime('%d/%m/%Y')}",
            )
            canvas.setStrokeColor(primary)
            canvas.setLineWidth(1.1)
            canvas.line(18 * mm, PAGE_H - 14.5 * mm, PAGE_W - 18 * mm, PAGE_H - 14.5 * mm)

        canvas.setStrokeColor(HexColor("#CBD5E1"))
        canvas.setLineWidth(0.5)
        canvas.line(18 * mm, 14.5 * mm, PAGE_W - 18 * mm, 14.5 * mm)
        canvas.setFillColor(dark)
        canvas.setFont("Helvetica-Bold", 6.8)
        canvas.drawString(18 * mm, 9.5 * mm, confidentiality.upper())
        canvas.setFillColor(HexColor("#64748B"))
        canvas.setFont("Helvetica", 6.8)
        canvas.drawCentredString(PAGE_W / 2, 9.5 * mm, f"INFORME V{report.version} · {report.reporter.full_name}")
        canvas.drawRightString(PAGE_W - 18 * mm, 9.5 * mm, f"PÁGINA {page}")
        canvas.restoreState()

    doc.addPageTemplates([PageTemplate(id="main", frames=[frame], onPage=header_footer)])

    story: list = []
    story.append(MatchHero(
        doc.width,
        club_name=club_name,
        ...
        rival_name=report.rival_team.name,
        primary=primary,
        dark=dark,
        logo=logo,
    ))
    story.append(Spacer(1, 7 * mm))
    story.append(_kpi_table(payload, styles, primary, dark))
    story.append(Spacer(1, 7 * mm))

    story.append(_section_heading("Resumen ejecutivo", styles, primary, dark, "Lectura del informador"))
    story.append(Spacer(1, 4 * mm))
    executive_box = Table([[
        Paragraph("IMPRESIÓN GENERAL", styles["callout_label"]),
        Paragraph(_rich_text(report.opponent_overview, "Sin impresión general registrada."), styles["callout_text"]),
    ]], colWidths=[31 * mm, 143 * mm])
    executive_box.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), HexColor("#F8FAFC")),
        ("BOX", (0, 0), (-1, -1), 0.6, HexColor("#CBD5E1")),
        ("LINEBEFORE", (0, 0), (0, 0), 3.2, primary),
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ("LEFTPADDING", (0, 0), (-1, -1), 5 * mm),
        ("RIGHTPADDING", (0, 0), (-1, -1), 5 * mm),
    ...
    if report.key_takeaways:
        story.append(Spacer(1, 4 * mm))
        key_box = Table([[
            Paragraph("IDEAS O NOMBRES A CONSERVAR", styles["callout_label"]),
            Paragraph(_rich_text(report.key_takeaways, ""), styles["body"]),
        ]], colWidths=[50 * mm, 124 * mm])
        key_box.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), _mix(primary, colors.white, 0.92)),
            ("BOX", (0, 0), (-1, -1), 0.5, _mix(primary, colors.white, 0.62)),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("LEFTPADDING", (0, 0), (-1, -1), 5 * mm),
            ("RIGHTPADDING", (0, 0), (-1, -1), 5 * mm),
    ...
    story.append(Spacer(1, 7 * mm))
    story.append(_section_heading("Jugadores a conservar en el radar", styles, primary, dark, "Selección automática del informe"))
    story.append(Spacer(1, 4 * mm))
    story.append(_watchlist_table(payload, styles, primary, dark))

    story.append(PageBreak())
    story.append(_section_heading("Contexto del partido", styles, primary, dark, "Alineaciones y minutos"))
    story.append(Spacer(1, 5 * mm))
    lineups = Table([[
        _lineup_card(report.own_team.name, match.home_formation if report.own_team_id == match.home_team_id else match.away_formation,
                     payload["own_players"], styles, primary, dark),
        _lineup_card(report.rival_team.name, match.home_formation if report.rival_team_id == match.home_team_id else match.away_formation,
                     payload["rival_players"], styles, primary, dark),
    ...
    if report.own_team_note:
        story.append(Spacer(1, 5 * mm))
        internal_note = Table([[
            Paragraph("NOTA INTERNA SOBRE NUESTRO EQUIPO", styles["callout_label"]),
            Paragraph(_rich_text(report.own_team_note, ""), styles["body"]),
        ]], colWidths=[48 * mm, 126 * mm])
        internal_note.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), HexColor("#F8FAFC")),
            ("BOX", (0, 0), (-1, -1), 0.5, HexColor("#CBD5E1")),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("LEFTPADDING", (0, 0), (-1, -1), 4 * mm),
        ...
        story.append(internal_note)

    story.append(PageBreak())
    story.append(_section_heading(f"Resumen de {report.rival_team.name}", styles, primary, dark, "Ordenado por valoración"))
    story.append(Spacer(1, 4 * mm))
    story.append(_player_overview_table(payload, styles, primary, dark))

    if payload["evaluated_players"]:
        story.append(Spacer(1, 8 * mm))
        story.append(_section_heading("Fichas individuales", styles, primary, dark, f"{len(payload['evaluated_players'])} jugadores evaluados"))
        story.append(Spacer(1, 6 * mm))
        for index, row in enumerate(payload["evaluated_players"]):
            story.append(CondPageBreak(70 * mm))
            card_parts = _player_card(row, styles, primary, dark)
            # Keep a normal card together when possible; detailed notes can continue beneath it.
            if row["evaluation"].detailed_note:
                story.append(KeepTogether(card_parts[:2]))
                story.extend(card_parts[2:])
            else:
                story.append(KeepTogether(card_parts))

    other_table = _other_observations_table(payload, styles, dark)
    own_table = _own_team_table(payload, styles, dark)
    if other_table or own_table:
        story.append(PageBreak())
        if other_table:
            story.append(_section_heading("Registro de observación", styles, primary, dark, "Jugadores sin nota completa"))
            story.append(Spacer(1, 4 * mm))
            story.append(other_table)
            story.append(Spacer(1, 8 * mm))
        if own_table:
            story.append(_section_heading(f"Notas internas · {report.own_team.name}", styles, primary, dark, "Bloque opcional"))
            story.append(Spacer(1, 4 * mm))
    ...
    story.append(Table([[
        Paragraph(
            f"Informe generado por <b>{_safe(report.reporter.full_name)}</b> · {generated_at} · "
...
def generate_report_pdf(session: Session, report_id: int, *, version: int | None = None, mode: str = "full") -> bytes:
    ...
    current_version = version or report.version
    ...
    def header_footer(canvas, current_doc):
        ...
        canvas.setFillColor(HexColor("#64748B")); canvas.setFont(_FONT_REGULAR, 6.2); canvas.drawCentredString(PAGE_W/2, 9.5*mm, f"V{current_version} · {mode.upper()} · {report.reporter.full_name}"); canvas.drawRightString(PAGE_W-18*mm, 9.5*mm, f"PÁGINA {page}")
        canvas.restoreState()
    doc.addPageTemplates([PageTemplate(id="main", frames=[frame], onPage=header_footer)])
