...
def test_403_release_contract_and_repair_migration_present():
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == "4.0.3"
    assert 'APP_VERSION = "4.0.3"' in (ROOT / "core/config.py").read_text(encoding="utf-8")
    assert (ROOT / "alembic/versions/0009_schema_repair_4_0_3.py").exists()
    assert 'HEAD_MIGRATION = "0009_schema_repair_4_0_3"' in (ROOT / "scripts/check_release_consistency.py").read_text(encoding="utf-8")
...
"""
    ...
    engine.dispose()
    assert version == "0009_schema_repair_4_0_3"
