...
def test_release_405_contract():
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == "4.0.5"
    assert 'APP_VERSION = "4.0.5"' in (ROOT / "core/config.py").read_text(encoding="utf-8")
    assert 'REPORTS_PAGE_API_VERSION = "4.0.5"' in (ROOT / "views/reports.py").read_text(encoding="utf-8")
