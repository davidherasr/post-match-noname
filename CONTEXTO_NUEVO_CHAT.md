> **ACTUALIZACIÓN VIGENTE · 4.4.4:** código fuente para futuros cambios = ZIP completo `NoName_Area_Tecnica_4.4.4.zip`, construido solo desde 4.4.3. Corrige `StreamlitWidgetAlreadyInstantiatedError` en DD mediante navegación lógica independiente y callbacks; lista de jugadores selecta frente a descubrimiento por nota ≥8/muestra; Informadores pueden registrar sin asignación lecturas *neutrales* voluntarias (NO postpartidos propios). Calendario con primera/actual, estado por temporada y filtros; archivo informes filtra/pagina en SQL, mejora Inicio, DD, Player360, ficha equipo, CSS, PDF largo. Sin migración nueva: Alembic 0015. No se ejecutó ninguna mutación sobre Supabase. Ver `UPGRADE_4_4_4.md` y `VALIDACION_4_4_4.md`; ejecución real Cloud, PostgreSQL y navegador siguen pendientes. Los bloques de versiones inferiores siguientes son histórico y NO deben reinterpretarse como código vigente.

> **ACTUALIZACIÓN 4.4.3 — NUEVA BASE CANDIDATA**: el ZIP completo `NoName_Area_Tecnica_4.4.3.zip` se construye exclusivamente desde `NoName_Area_Tecnica_4.4.2.zip`. VERSION/APP_VERSION/reports = 4.4.3 y Alembic head = `0015_observation_requests_4_4_3`. Incluye lectura breve por defecto con botones 1–10 y modo detallado alternativo, entrega directa a DD y regreso a Inicio, seguimiento solo por check autorizado no preseleccionado, archivo Informes en menú, DD con mesa/jugadores de interés/peticiones voluntarias, destinatarios Informador, avisos por partido verificado/plantilla posible, respuesta con enlace a evidencia original sin duplicar notas. También corrige edición neutral de señales vinculadas, ficha de externo y asignación de rol deportivo sin estado Observado artificial. Nueva migración aditiva de tres tablas: respaldar PostgreSQL + bucket y validar replica antes de Cloud. Tests locales y SQLite no validan navegador, Cloud ni PostgreSQL real. Lee primero `UPGRADE_4_4_3.md`, `VALIDACION_4_4_3.md`, README y CHANGELOG. Datos históricos intactos por la migración; nunca mezclas archivos, stamp ni resetees Supabase. Este bloque prevalece sobre notas históricas 4.4.2 y anteriores que siguen abajo a modo de archivo.

> **ACTUALIZACIÓN PRIORITARIA — 4.4.2 (candidata, sin desplegar):** El código fuente para continuar es exclusivamente `NoName_Area_Tecnica_4.4.2.zip` al verificar su integridad; procede únicamente del ZIP completo 4.4.1. Flujo B sigue: entrega de informe propio → incorporación inmediata, sin aprobar. Si el autor tiene `can_track_players`, tras entregar recibe un paso opcional para seleccionar rivales puntuados y enriquecer sin segunda nota: la valoración es la fuente única y una `ScoutObservation` enlazada por FK guarda detalles, presentada como un único evento en Player 360/PDF. Nota ≥8 solo preselección sugerida, nunca acción automática. Reutilización por partido, jugador y autor evita repetición en guardados sucesivos; filas históricas no se eliminan ni fusionan en BD durante migración. DD tiene actividad identificando autor/futbolista, acceso al Player 360, decisiones deportivas y borrado auditado SOLO de duplicados, conservando registro vinculado y al menos un evento. Migración NUEVA 0014 nullable+FK+índice único, desde 0013, requiere backup real y validación PostgreSQL/Cloud; no tocar Supabase por intuición. Ver README, `UPGRADE_4_4_2.md`, `VALIDACION_4_4_2.md` y pruebas de regresión. Las entradas 4.4.1, 4.2.3.x y 4.2.2 siguientes son contexto HISTÓRICO y NO describen la release actual. Nunca mezclar directorios ni correr borrado/desduplicado al desplegar.

> **Última release candidata 4.4.1:** construida exclusivamente sobre ZIP íntegro 4.4. Notas 1–10/Sin evaluar mediante pulsación, conservando decimales históricos; entrega redirige a Inicio y limpia sesión del editor; Informador puede rechazar asignaciones sin borrar borradores (estado `declined`, excluido de tareas y contadores); Admin puede reactivar; DD solo elabora informes con rol Informador adicional y voluntariado explícito en partido propio publicado. Sin nueva migración: head 0013. 140 tests y migraciones SQLite aisladas OK, no probado en Cloud/Supabase. Ver UPGRADE_4_4_1.md y VALIDACION_4_4_1.md. Código base futuro = ZIP 4.4.1 cuando se verifique, no mezclar con versiones previas.

> **Última release candidata 4.2.3.2 (16/09/2026):** branding público «No Name · Área Técnica»; borrado permanente por lotes desde Administración → Datos para temporadas/partidos/equipos/jugadores, con preview del grafo FK, confirmación de backup, transacción, auditoría, selección en español y recomendación de jugadores huérfanos NO automática. Los PDF/archivos del bucket no se eliminan físicamente con las filas de BD. Asignación de Informadores arreglada: checkboxes fuera del form, botones de selección traducidos y guardado activo con validación. Exportación tabular incorpora todas las tablas pero NO es copia restaurable ni reemplaza pg_dump. **Head sigue 0013, sin nueva migración; 128 tests locales y fresh/upgrade SQLite aislado OK; producción Cloud/Supabase no validada ni modificada.** Usar solo ZIP completo 4.2.3.2 como base de código una vez verificado; `UPGRADE_4_2_3_2.md` y `VALIDACION_4_2_3_2.md` explican procedimiento. Las versiones previas son histórico. El borrado definitivo es una nueva operación ADMIN EXPLÍCITA excepcional; no contradice la política de NO eliminar datos automáticamente al desplegar ni hacer resets.
