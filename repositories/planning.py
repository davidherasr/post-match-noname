...
def create_observation(
    ...
    *,
    ...
    if match_id is not None:
        # Preserve the legacy public API while preventing additional identical
        # sporting events from being generated by callers outside today's UI.
        from repositories.tracking import get_or_create_match_observation
        item, created = get_or_create_match_observation(session,
            player_id=int(player_id), author_id=int(reviewer_id), match_id=int(match_id))
        if created:
            item.source_type = source_type
            item.observation_level = level
            item.mission_id = mission_id
            item.model_role_id = int(model_role_id) if model_role_id else None
            audit(session, reviewer_id, "create_scout_observation", "scout_observation", item.id,
                  detail=f"player={player_id}; match={match_id}; mission={mission_id}")
        return item
...
def save_quick_match_observations(
    ...
    for row in rows:
        ...
        if not player_id or rating is None or float(rating) <= 0:
            continue
        # One match/player/observer event even if an older endpoint is invoked
        # repeatedly. No duplicate grade can be created by a quick sweep.
        from repositories.tracking import get_or_create_match_observation
        item, created = get_or_create_match_observation(session, player_id=player_id,
            author_id=reviewer_id, match_id=match.id)
        if created:
            item.source_type = "match_scan"
            item.observation_level = "scan"
            item.mission_id = mission.id if mission else None
        item.status = "submitted"
        item.observed_at = item.observed_at or UTC_NOW()
        # A postmatch-linked grade is authoritative and cannot be overwritten.
        if item.player_evaluation_id is None:
            item.general_rating = float(rating)
        item.observed_position = row.get("observed_position") or item.observed_position
        item.summary = (row.get("summary") or "").strip() or item.summary
        item.recommendation = row.get("recommendation") or item.recommendation or "Sin conclusión"
        item.submitted_at = item.submitted_at or UTC_NOW()
        item.updated_at = UTC_NOW()
...
def scouting_evidence_many(session: Session, player_ids: Sequence[int], *, season_id: int | None = None) -> dict[int, dict]:
    ...
    scout_stmt = (
        select(ScoutedPlayerProfile.player_id, ScoutObservation.id, ScoutObservation.reviewer_id,
               ScoutObservation.match_id, ScoutObservation.observed_at)
    ...
    unique_events: dict[int, dict[tuple, tuple]] = {}
    for pid, obs_id, reviewer_id, match_id, observed_at in session.execute(scout_stmt).all():
        # Historical duplicate submissions have a single logical event for
        # the same player, observed match and author. Standalone assessments
        # are independent events and retain their own identity.
        event_key = (int(reviewer_id), int(match_id)) if match_id is not None else ("standalone", int(obs_id))
        group = unique_events.setdefault(int(pid), {})
        old = group.get(event_key)
        if old is None or (observed_at and (old[2] is None or observed_at > old[2])):
            group[event_key] = (int(reviewer_id), match_id, observed_at)
    for pid, events in unique_events.items():
        row = result[int(pid)]
        count = len(events)
        scouts = len({event[0] for event in events.values()})
        matches = len({event[1] for event in events.values() if event[1] is not None})
        last = max((event[2] for event in events.values() if event[2] is not None), default=None)
        row.update(
            specific_observations=count, specific_scouts=scouts, specific_matches=matches, specific_last=last,
