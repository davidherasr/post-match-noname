...
def _director_profile(profile, user: dict) -> None:
    ...
    with session_scope() as session:
        ...
        reviews = scout_repo.list_reviews(session, profile.id)
        positions = league_repo.observed_position_counts(session, player.id)
    reporters = [u for u in users if u.role in {"reporter", "director", "admin"}]
    labels = {u.id: f"{u.full_name} · {u.role}" for u in reporters}
    ...
    with st.form(f"dd_scout_profile_{profile.id}"):
        ...
        current_roles = roles_for(model_position)
    ...
    if save or approve:
        ...
        st.rerun()
    if reviews:
        st.markdown("#### Informes scout recibidos")
