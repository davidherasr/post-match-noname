...
def render(user: dict) -> None:
    ...
    st.caption('Todas las operaciones se realizan por ID y quedan auditadas. No se eliminan, fusionan ni modifican informes o valoraciones.')
    ...
    if own and (configured != str(own.id) or flagged_ids != {own.id}):
        st.warning(f'Referencias desincronizadas: own_team_id={configured!r}, marcados={sorted(flagged_ids)}. Pulsa Confirmar equipo propio para sincronizar las dos fuentes.')
    st.info(f"Equipo identificado actualmente: {own.name} · ID {own.id}" if own else 'No hay equipo propio válido. Selecciona el registro real ya existente.')
    candidates = [t.id for t in teams if t.active and not t.is_test and t.archived_at is None]
    if candidates:
        target_id = st.selectbox('Seleccionar equipo propio por ID', candidates,
                                 index=candidates.index(own.id) if own and own.id in candidates else 0,
                                 format_func=lambda tid: f'{by_id[tid].name} · ID {tid}', key='gov_own_423')
        ...
        if st.button('Confirmar equipo propio', disabled=bool(own and own.id == target_id and configured == str(target_id) and flagged_ids == {target_id}), key='gov_set_own_423'):
            try:
                ...
                st.success('Equipo propio sincronizado por ID.'); st.rerun()
    ...
    team_id = st.selectbox('Inspeccionar equipo', [t.id for t in teams],
                           format_func=lambda tid: f'{by_id[tid].name} · ID {tid}' + (' · ARCHIVADO' if by_id[tid].archived_at else '') + (' · PRUEBA' if by_id[tid].is_test else ''),
                           key='gov_team_423')
    with session_scope() as session:
    ...
    st.write(f'**{by_id[team_id].name} · ID {team_id}**')
    ...
    if is_own:
        ...
        if team_is_test:
            confirm_team = st.checkbox(f'He verificado el ID {team_id} y sus dependencias; quiero archivarlo sin borrar datos.', key=f'gov_confirm_team_{team_id}')
    ...
    st.caption(f'{len(related)} encuentros asociados al equipo seleccionado. Solo se modifica el ID que confirmes.')
    ...
    match_id = st.selectbox('Inspeccionar encuentro', list(matches),
        format_func=lambda mid: f'ID {mid} · {matches[mid].match_date} · {matches[mid].home_team.name} - {matches[mid].away_team.name}' + (' · ARCHIVADO' if matches[mid].deleted_at else '') + (' · PRUEBA' if matches[mid].is_test else ''),
    ...
    st.write(f'**Partido ID {match_id}** · {match.match_date} · {match.round_name} · {match.status}')
    ...
    if archived:
        ...
        if is_test:
            confirm = st.checkbox(f'He comprobado partido ID {match_id}, fecha, equipos y dependencias; autorizo su archivado reversible.', key=f'gov_confirm_match_{match_id}')
